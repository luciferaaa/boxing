...
class MobliebaseController extends AppframeController{
     ...
     * 获取关注微信人的详细信息
     **/
    protected function _getWeiXinUser(){
        $weixin_user_model = M('weixin_user');
        if(!empty($_COOKIE['openid'])){
            $user_msg['openid'] = $_COOKIE['openid'];
            $weixin_user_data = $weixin_user_model->where(array('openid'=>$_COOKIE['openid']))->find();
            ...
            $weixin_user_data = $weixin_user_model->where(array('openid'=>$_COOKIE['openid']))->find();
            $user_msg_r = $this->_getWeiXinMsg($data['weixin_access_token'],$openid['openid']);
            $time = time() + 2592000;
     ...
     * 获取jsapi配置信息
     **/
    protected function _getSignPackage() {
     ...
     *
     * */
    public function _getWeiXinMsg($token,$openid) {
     ...
     */
    protected function _MakeSign($array){
        //签名步骤一：按字典序排序参数
        ksort($array);
        $string = $this->_ToUrlParam($array);
        //签名步骤二：在string后加入KEY
     ...
     * 格式化参数格式化成url参数
     */
    protected function _ToUrlParam($data){
        $buff = "";
        foreach ($data as $k => $v)
        {
